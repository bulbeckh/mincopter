	...
	public:
		...
		virtual void inav_reset(void) {
			for (uint8_t i=0;i<3;i++) {
				...
				_inav_state->_position[i] = 0.0f;
				_inav_state->_velocity[i] = 0.0f;
			}
			return;
		}
		...
		virtual void inav_reset_pos_vel(Vector3f pos, Vector3f vel) {
			_inav_state->_position[0] = pos.x;
			_inav_state->_position[1] = pos.y;
			_inav_state->_position[2] = pos.z;

			_inav_state->_velocity[0] = vel.x;
			_inav_state->_velocity[1] = vel.y;
			_inav_state->_velocity[2] = vel.z;

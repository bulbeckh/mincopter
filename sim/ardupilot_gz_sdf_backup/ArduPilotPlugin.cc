...
class Control
{
  ...
  public: double servo_min = 1200.0;
  ...
  public: double servo_max = 1900.0;
...
class gz::sim::systems::ArduPilotPluginPrivate
{
  ...
  public: bool compassMsgValid{false};
  public: std::mutex compassMsgMutex;
  public: bool compassInitialized{false};

  public: void CompassCb(const gz::msgs::Magnetometer &_msg)
  {
    std::lock_guard<std::mutex> lock(this->compassMsgMutex);
    compassMsg = _msg;
    compassMsgValid = true;
  }

  public: gz::msgs::FluidPressure baroMsg;
  public: bool baroMsgValid{false};
  public: std::mutex baroMsgMutex;
  public: bool baroInitialized{false};

  public: void BarometerCb(const gz::msgs::FluidPressure &_msg)
  {
    std::lock_guard<std::mutex> lock(this->baroMsgMutex);
    baroMsg = _msg;
    baroMsgValid = true;
  }
    ...
    const gz::sim::EntityComponentManager &_ecm) const
{
    ...
    { 
        std::lock_guard<std::mutex> lock(this->dataPtr->baroMsgMutex);
        // Wait until we've received a valid message.
        if (!this->dataPtr->baroMsgValid)
        {
            return;
        }
	...
	static bool gotfirstns=false;
	static double alt_off;
	if (!gotfirstns) {
		alt_off = navsatMsg.altitude();
		gotfirstns=true;
	}
    ...
    */
	...
	// IMU expressed in bodyG so need to convert both gyro and accel to bodyA
    ...
    gz::math::Vector3d compass_field{
		compassMsg.field_tesla().x(),
		compassMsg.field_tesla().y(),
		compassMsg.field_tesla().z()
	};

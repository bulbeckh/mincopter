...
class CompassSampleRingBuffer {
public:
    CompassSampleRingBuffer() : write_index_(0), read_index_(0), dropped_(0) {}

    bool push(const CompassSample &sample) {
...
private:
    CompassSample buffer_[Capacity];
    volatile size_t write_index_;
    volatile size_t read_index_;
    size_t dropped_;
};

struct CompassTaskConfig {
    uint32_t notification_timeout_ms;
    uint32_t polling_period_ms;
};

template <size_t BufferCapacity>
struct CompassTaskContext {
    mc_rtos_hal::Hal &hal;
    CompassDevice &device;
    CompassTaskConfig config;
    CompassSampleRingBuffer<BufferCapacity> *ring;
    void *task_handle;
    CompassTaskStats stats;
};
...
public:
    ...
    static void task_entry(void *context) {
        run(*static_cast<CompassTaskContext<BufferCapacity> *>(context));
    }

    template <size_t BufferCapacity>
    static bool create(CompassTaskContext<BufferCapacity> &context,
                       const char *name,
                       uint16_t stack_words,
                       mc_rtos_hal::TaskPriority priority) {
        mc_rtos_hal::TaskConfig task_config{};
        task_config.name = name;
        task_config.entry = &CompassTask::task_entry<BufferCapacity>;
    ...
    static void data_ready_isr(void *context) {
        auto &task_context = *static_cast<CompassTaskContext<BufferCapacity> *>(context);
...
private:
    template <size_t BufferCapacity>
    static void run(CompassTaskContext<BufferCapacity> &context) {
        ...
        if (context.device.has_data_ready_irq()) {
            ...
            if (pin != nullptr) {
                ...
                irq_cfg.callback = &CompassTask::data_ready_isr<BufferCapacity>;
